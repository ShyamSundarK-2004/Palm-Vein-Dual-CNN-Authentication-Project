...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for raw, clahe, labels in tqdm(loader, desc="Train"):
        ...
        clahe, labels = clahe.to(device), labels.to(device)
        ...
        total_loss += loss.item() * clahe.size(0)
...
def main():
    ...
    model = RawResNet18(num_classes=train_ds.num_classes).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"[CLAHE] Epoch {epoch}/{EPOCHS} | "
        ...
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), "outputs/clahe_model.pth")

    print("🔥 Best CLAHE Accuracy:", best_acc)
